...
function calculateDashboard(profile: UserProfile): DashboardState {
  ...
  const costMap: Record<string, number> = {
    'USA': 65000, 'UK': 45000, 'Canada': 40000, 'Australia': 42000,
    'Germany': 8000, 'Netherlands': 18000, 'Singapore': 35000, 'Ireland': 28000, 'India': 5000
  };
  const primaryCountry = countries[0] || 'USA';
  const estimatedCost = costMap[primaryCountry] || 45000;
  const salaryMap: Record<string, number> = {
    'Computer Science': 95000, 'Data Science': 90000, 'Finance': 85000,
    'Engineering': 80000, 'Business': 75000, 'Healthcare': 78000, 'Law': 82000
  };
  const baseSalary = salaryMap[profile.field] || 75000;
  const paybackYears = (estimatedCost / baseSalary).toFixed(1);
  const loanScore = Math.min(99, Math.round(60 + (gpa * 4) + (budget > 40000 ? 10 : 0)));
  const admissionChance = Math.min(98, Math.round(50 + (gpa * 5)));
  const riskScore: 'Safe' | 'Moderate' | 'Risky' =
    gpa >= 8 && budget >= 30000 ? 'Safe' : gpa >= 6.5 ? 'Moderate' : 'Risky';
  return {
    riskScore,
    aiInsight: profile.gpa > 0
      ? `Based on your ${gpa} GPA in ${profile.field}, ${primaryCountry} offers strong ROI with ~${paybackYears} year payback. Your profile is ${riskScore.toLowerCase()} risk.`
      : "Complete your profile to get personalized AI insights for your education journey.",
    roi: { cost: `$${(estimatedCost / 1000).toFixed(0)}k`, salary: `$${(baseSalary / 1000).toFixed(0)}k`, payback: `${paybackYears} Years` },
    admissionChance,
    loanEligibility: loanScore,
    incomeEMI: { income: Math.round(baseSalary / 12), emi: Math.round(estimatedCost / 120) },
    worstCaseImpact: `In a downturn, your ${primaryCountry} degree provides ~18 months job search buffer with ${riskScore.toLowerCase()} financial exposure.`
  };
}
...
const INITIAL_PROFILE: UserProfile = {
  name: "", gpa: 0, field: "Computer Science", goal: "MS in Computer Science",
  budget: 50000, preferredCountries: ["USA", "Germany"], educationLevel: "Postgraduate"
};
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            const saved = docSnap.data() as UserProfile;
            setProfile({ ...saved, name });
            setDashboard(calculateDashboard({ ...saved, name }));
      ...
      {currentView === 'dashboard' && (
        <div className="flex h-screen bg-slate-50 font-sans text-slate-900 overflow-hidden">
          <nav className="w-20 bg-slate-900 flex flex-col items-center py-8 gap-8 shrink-0 z-50">
            ...
            <div className="mt-auto px-4 flex flex-col items-center gap-3">
              {profileSaved && <div className="w-2 h-2 bg-emerald-400 rounded-full" title="Profile saved" />}
              <button
          ...
          <div className="flex-1 flex flex-col relative overflow-hidden">
            <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200 px-10 flex items-center justify-between sticky top-0 z-30">
              ...
              <div className="flex items-center gap-6">
                ...
                <button onClick={logout} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 rounded-xl text-xs font-bold transition-all">
                  Logout
                </button>
              </div>
            </header>
                ...
                >
                  ...
                  <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
                    <div className="lg:col-span-7 space-y-8">
                      ...
                      <div className="p-6 bg-white/10 border border-white/10 rounded-3xl backdrop-blur-2xl">
                        <div className="flex gap-4 items-start">
                          <div className="p-3 bg-white/10 rounded-2xl text-indigo-200"><Sparkles size={22} /></div>
                    ...
                    <div className="lg:col-span-5 grid grid-cols-2 gap-5">
                      ...
                      <StatCard label="Salary Cap" value={dashboard.roi.salary} detail="Post-5 Years" />
                ...
                <section className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                  <div className="lg:col-span-8 space-y-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <VisualInsightCard title="Financial Matrix" icon={<Wallet size={20} className="text-emerald-500" />} tags={['Personalized', 'Market Sync']}>
                        <div className="space-y-6">
                          <div className="grid grid-cols-2 gap-5">
                            <div className="p-5 bg-slate-50 rounded-3xl border border-slate-100">
                              <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">Est. Cost</p>
                              <p className="text-2xl font-bold font-mono tracking-tighter">{dashboard.roi.cost}</p>
                            </div>
                            <div className="p-5 bg-emerald-50 rounded-3xl border border-emerald-100">
                              <p className="text-[10px] text-emerald-600 font-bold uppercase mb-2">Avg Salary</p>
                              <p className="text-2xl font-bold font-mono tracking-tighter text-emerald-700">{dashboard.roi.salary}</p>
                            </div>
                          </div>
                          ...
                          <div className="p-5 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between">
                            <span className="text-xs font-bold text-slate-500 uppercase tracking-tight">Est. EMI/month</span>
                            <span className="text-lg font-bold text-slate-700">${dashboard.incomeEMI.emi.toLocaleString()}</span>
                          </div>
                        </div>
                      </VisualInsightCard>

                      <VisualInsightCard title="Loan Eligibility" icon={<ShieldCheck size={20} className="text-indigo-600" />} tags={['Low EMI', 'Pre-qualified']}>
                        <div className="flex flex-col items-center justify-center p-4">
                          ...
                          <div className="mt-4 w-full p-4 bg-slate-50 rounded-2xl border border-slate-100 text-center">
                            <p className="text-xs text-slate-500 font-medium">Monthly Income at Year 1</p>
                            <p className="text-lg font-bold text-indigo-600 mt-1">${dashboard.incomeEMI.income.toLocaleString()}</p>
                          </div>
                        </div>
                      </VisualInsightCard>
                    </div>
                  ...
                  <div className="lg:col-span-4 space-y-10">
                    <div className="bg-slate-900 rounded-[40px] p-10 text-white shadow-2xl shadow-indigo-900/20 h-full flex flex-col">
                      <div className="flex items-center justify-between mb-10">
                        ...
                        <div className="p-2 bg-white/5 rounded-xl text-white/40"><History size={16} /></div>
                      </div>
                      <div className="flex-1 space-y-10 relative">
                        ...
                        {history.slice(0, 5).map((item, idx) => (
                          <div key={item.id} className="relative flex gap-8 group cursor-default">
                            <div className={cn("relative z-10 w-8 h-8 rounded-full flex items-center justify-center border-[3px] border-slate-900 transition-all",
                      ...
                      <div className="mt-10 p-6 bg-indigo-600/10 rounded-[32px] border border-white/10 group cursor-pointer hover:bg-indigo-600/20 transition-all"
                        onClick={() => handleSend("What is my next milestone to prepare for studying abroad?")}>
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar bg-slate-50/30">
                    {messages.length === 0 && (
                      <div className="h-full flex flex-col items-center justify-center text-center px-10 py-16">
                        ...
                        <div className="mt-6 flex flex-col gap-2 w-full">
                          {["What's my best university match?", "Simulate worst case scenario", "Show loan repayment plan"].map(q => (
                            <button key={q} onClick={() => handleSend(q)}
                              className="w-full text-left px-4 py-3 bg-white border border-slate-200 rounded-2xl text-xs font-medium text-slate-600 hover:border-indigo-300 hover:text-indigo-600 transition-all">
                              {q}
                            </button>
                          ))}
                        </div>
                    ...
                    {messages.map((msg, idx) => (
                      <div key={idx} className={cn("flex gap-4 max-w-[90%]", msg.role === 'user' ? "ml-auto flex-row-reverse" : "mr-auto")}>
                        <div className={cn("shrink-0 w-8 h-8 rounded-xl flex items-center justify-center shadow-sm mt-1",
                        ...
                        <div className={cn("p-5 text-sm leading-relaxed rounded-3xl shadow-sm",
        ...
        <div className="bg-white/10 backdrop-blur-2xl rounded-[32px] border border-white/20 p-8 shadow-2xl">
          <div className="flex bg-white/5 rounded-2xl p-1 mb-8 border border-white/10">
            <button onClick={() => { setTab('login'); setError(''); setSuccess(''); }}
              className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${tab === 'login' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-white/50 hover:text-white'}`}>Login</button>
            <button onClick={() => { setTab('register'); setError(''); setSuccess(''); }}
              className={`flex-1 py-3 rounded-xl text-sm font-bold transition-all ${tab === 'register' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-white/50 hover:text-white'}`}>Register</button>
          </div>
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleEmailAuth()} placeholder="you@example.com"
                className="w-full bg-white/10 border border-white/20 rounded-2xl px-5 py-4 text-white placeholder-white/30 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
            </div>
            <div>
              <label className="text-xs font-bold text-white/50 uppercase tracking-widest mb-2 block">Password</label>
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleEmailAuth()} placeholder="Min. 6 characters"
                className="w-full bg-white/10 border border-white/20 rounded-2xl px-5 py-4 text-white placeholder-white/30 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" />
            </div>
            {success && (
...
function RiskGauge({ score }: { score: 'Safe' | 'Moderate' | 'Risky' }) {
  ...
  return (
    <div className={cn("p-6 bg-white/10 rounded-3xl border backdrop-blur-3xl flex flex-col justify-between", configs[score].border)}>
      ...
      <div className={cn("px-5 py-2.5 rounded-2xl text-xl font-black italic tracking-tight w-fit", configs[score].bg, configs[score].text)}>{score}</div>
...
function VisualInsightCard({ title, icon, children, tags }: { title: string, icon: React.ReactNode, children: React.ReactNode, tags?: string[] }) {
  return (
    <div className="bg-white rounded-[40px] p-8 border border-slate-200 shadow-xl shadow-slate-200/40 flex flex-col h-full group hover:border-indigo-600/30 transition-all hover:shadow-indigo-600/5">
      <div className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-[22px] bg-slate-50 flex items-center justify-center border border-slate-100 group-hover:bg-indigo-50 group-hover:border-indigo-100 transition-all">{icon}</div>
          <h3 className="font-bold text-lg text-slate-800 tracking-tight">{title}</h3>
        </div>
        <div className="flex gap-2">
          {tags?.map(tag => (
            <span key={tag} className="px-3 py-1.5 rounded-full bg-slate-50 text-slate-400 text-[10px] font-bold uppercase tracking-widest border border-slate-100 transition-all group-hover:bg-indigo-50/50 group-hover:text-indigo-400">{tag}</span>
          ))}
        </div>
      </div>
...
function QuickAction({ icon, label, onClick, className }: { icon: React.ReactNode, label: string, onClick: () => void, className?: string }) {
  return (
    <button onClick={onClick}
      ...
      <div className="w-14 h-14 bg-slate-50 rounded-[22px] flex items-center justify-center transition-all group-hover:bg-indigo-600 text-slate-400 group-hover:text-white group-hover:rotate-12">{icon}</div>
